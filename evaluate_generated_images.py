...
folders = [
    ...
    "/home/drozdova/projects/ska-project/diffusion-true-ska/results/direct_256",
    "/home/drozdova/projects/ska-project/diffusion-true-ska/results/repeat_256",
    ...
    "/home/drozdova/projects/ska-project/galaxy_zoo_generation_diffusion/results/direct_re_im_simple",
    ]
partition = "val"
additional_line = ""
for folder in folders:
    ...
    # ra, dec, SNR, ?, intensity, ?, ?

    if additional_line == "_test=gen":
        test_generated_images = test_images
    ...
    def true_itrasnform(true):
        true_back = true * 0.5 + 0.5
        true_back = true_back * (0.7063881)
    ...
    def match_points(coords_from_true, coords_from_gen, eps, idx=2, fixed_snr=None, snr_idx=3):
        ...
        dist_matrix = np.sqrt(
            np.sum((coords_from_true[:, :2, np.newaxis] - coords_from_gen[:, :2, np.newaxis].T) ** 2, axis=1))
    ...
    def im_reshape(downsampled_array):
        ...
        #resize(real_input, (512, 512), order=3, mode='edge', anti_aliasing=True)
        #zoom(real_input, zoom=zoom_nb, order=3)
    ...
    def run_test_experiment(
        ...
        zoom_nb=4,
            ...
            real_input = test_images[i]
            if apply_itransform:
                real_input = true_itrasnform(real_input)
            real_input = real_input[:,:,0]
            real_input = im_reshape(real_input)
            ...
            gt_sources = np.hstack(
                (
                    gt_sources[:,:2], 
                    true_sources[:,4].reshape(-1,1), #something which look like intensity
            ...
            label = np.stack(
                [
                    test_labels1[i]*(std_vectors+1e-10)+mean_vectors,
                    test_labels2[i]*np.pi,
                ]
            )

            #artificial_dirty_im = fourier_upsample(label, use_casa=True)
            ...
            for j in range(repeat_images):
                ...
                gen_im = gen_im[:,:,0]
                #gen_im = zoom(gen_im, zoom=zoom_nb, order=3)
                gen_im = im_reshape(gen_im)
                #plot_generated_images(im, gen_im, noisy_im)
            ...
            # b is None if the sequencies are empty
            if np.max(gen_im) < 1e-10:
            ...
            #brightness = np.stack((sources_from_true[:,3], sources_from_generated[:,3]), axis=0).T
            #brightnesses.extend(brightness.tolist())
            ...
            if sources_from_generated is None:
                return None
            brightness, tp, fp, fn = match_points(sources_from_true, sources_from_generated, 1, idx=2)
            ...
            if verbose:    
                # Create a DataFrame
                #print(true_sources)
                #true_sources[:,5:6] = true_sources[:,5:6] * 2.3548 
                #print(true_sources)
    ...
    def test(
        ...
        if len(brightnesses) > 0:
            ...
            if plot_brightness:
                ...
                plt.savefig(f"{folder}/brightness_{partition}_{i}{additional_line}.jpg",)
                plt.close()
                ...
                plt.savefig(f"{folder}/majors_{partition}_{i}{additional_line}.jpg",)
                plt.close()
                ...
                plt.savefig(f"{folder}/minors_{partition}_{i}{additional_line}.jpg",)
                plt.close()
